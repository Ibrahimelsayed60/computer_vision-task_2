...
class canny :
    ...
    def __init__(self,image,closed = True):
        ...
        self.smooth_img = smooth(self.gray, gauss)
        fx = ApplyMask(self.smooth_img, gx)
        fy = ApplyMask(self.smooth_img, gy)
        self.mag = Gradient_Magnitude(fx, fy)
        self.mag = mag.astype(int)
        self.Angle = Gradient_Direction(fx, fy)
        self.quantized = Digitize_angle(self.Angle)
        self.nms = Non_Max_Supp(quantized, self.Angle, self.mag)
        self.threshold = _double_thresholding(self.nms, 30, 60)
        self.hys = _hysteresis(self.threshold)
